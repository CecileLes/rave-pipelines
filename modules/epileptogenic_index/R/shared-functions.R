...
compute_ER <- function(load_electrodes, repository, time_window, condition) {
  ...
  ###################################################
  # Get signal data for each electrode
  ...
  #### modify below to run using variables from above ###
  ...
  ermaster <- as.matrix(ermaster)
  unmaster <- as.matrix(unmaster)
  return(list(ermaster=ermaster,unmaster=unmaster))
